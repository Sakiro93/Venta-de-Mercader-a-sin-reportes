...
Public Class Man_Color
    ...
    Public Sub cargar(ByVal Buscar As String)
        Dg.Rows.Clear()
        For Each col In Logicadatos.buscarCol(CBOpcion.SelectedIndex, Buscar)
            Dg.Rows.Add(col.col_ID, col.col_Nombre)
        Next
    End Sub
    Public Sub limpiar()
        ErrDatos.Clear()
    ...
    End Sub
    Private Sub Man_Color_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        limpiar()
    ...
    Private Sub BtnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnGuardar.Click
        Dim datos = New EntColor(Cod, TxtDescripcion.Text)
        If MessageBox.Show("¿Desea guardar estos datos en el Sistema?", "Sistema de Ventas de Mercadería", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
            If Metodo.ValidaEntrada(ErrDatos, GB_Ingresar) Then
                If grabarG = "N" Then
                    Logicadatos.ingresarCol(datos)
                Else
                    Logicadatos.modificarCol(datos)
                End If
                limpiar()
